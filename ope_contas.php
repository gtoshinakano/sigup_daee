...
if (!isset($_GET['notas'])) {
    if (!isset($_GET['ano_ref'], $_GET['mes_ref'], $_GET['emp_id']) || $_GET['ano_ref'] < 2012 || $_GET['mes_ref'] < 1 || $_GET['mes_ref'] > 12 || $_GET['ano_ref'] > Date('Y')) { //se não tem get
        ...
        if ($nivel != 1) {
            $sql = "SELECT  emp.nome emp_nome , emp.id emp_id
                FROM daee_notas n, daee_uddc uc, daee_udds uo, daee_contratos c, sys_cidade cid, sys_empresas emp 
                WHERE n.uc=uc.id AND uc.contrato=c.id AND uc.cidade=cid.id AND c.empresa=emp.id AND uc.uo=uo.id AND c.permissao=$nivel
                group by  emp_nome DESC  
                order by  emp_nome   ";
        } else {
            $sql = "SELECT  emp.nome emp_nome , emp.id emp_id
                FROM daee_notas n, daee_uddc uc, daee_udds uo, daee_contratos c, sys_cidade cid, sys_empresas emp 
        ...
        while ($res = mysql_fetch_array($query)) {
            $tpl->EMP = $res['emp_nome'];
            $tpl->EMP_ID = $res['emp_id'];
            $tpl->block('EACH_EMP');
        }
