							...
							foreach($var as $tipo => $valores){
								...
								foreach($valores as $mes =>$valor){
									...
									if(!isset($uoTotal[$ano][$tipo][$mes])){
                                        ...
                                        /************************************
                                        Calcular Médias PARA O ANO EM QUESTÃO
                                         ***********************************/
                                        $med_pago = round(array_sum($uoTotal[$start]['valor']) / getDivisorParaMedia($uoTotal[$start]['valor']) * 100) / 100;
                                        $med_cons = round(array_sum($uoTotal[$start]['consumo'])/ getDivisorParaMedia($uoTotal[$start]['valor']) * 100) / 100;
